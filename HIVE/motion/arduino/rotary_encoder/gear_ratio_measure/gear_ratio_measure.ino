...
//const float ENC_0_GEAR_RATIO = 1.1786;
...
void setup() {
    Serial.begin(38400);
    Serial.println("PROGRAM: gear_ratio_measure.ino");
    Serial.println("UPLOAD DATE: 2024 NOV 05");

    Serial.println("");
    Serial.println("Directions:");
    Serial.println("Attach geared encoder to LEFT channel");
    Serial.println("Attach 1:1 encoder to RIGHT channel");
    Serial.println("");
    ...
    Serial.println("Time elapsed [s], Command Value, omega_enc_L_geared [rad/s], omega_enc_R_true [rad/s], ratio (true/gear)");
...
void loop(){
    ...
    if (curr_sensorTimer - prev_sensorTimer > sensorInterval) {
        ...
        // calculate ratio
        gear_ratio = omega_enc_1/omega_enc_0;
